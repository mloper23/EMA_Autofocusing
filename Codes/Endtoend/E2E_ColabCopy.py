...
epochs = 500
batch_size = 10
title = f"8E_E2E_{epochs}_Epochs"
...
x_set = y_set = [name for name in x_set if name in y_set]
...
sequence_val = LoadImagesValidation(x_test, y_test, data_dir, 1, data_real)
...
with strategy.scope():
    ...
    unet.compile(optimizer="sgd", loss="mse")
...
history = unet.fit(sequence_train, epochs=epochs, validation_data=sequence_val,
